local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/youssefDYDYYDYD/yousYYS1tp/refs/heads/main/V2"))()
local ThemeManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/youssefDYDYYDYD/youssS2TP/refs/heads/main/goatedd"))()
local SaveManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/youssefDYDYYDYD/Yoused/refs/heads/main/SaveManager.lua"))()
local Window = Library:CreateWindow({
 Title = 'Tappeed V2 By Youss | ',
    Center = true,
    AutoShow = true,
    TabPadding = 8,
    MenuFadeTime = 0.2,
...
LeftGroupBox = Tabs.Main:AddLeftGroupbox('Camlock')
    ...
    local previewColor = Color3.fromRGB(255, 255, 255) -- Default preview color
    local lockedHighlightColor = Color3.fromRGB(255, 255, 255) -- Default locked highlight color
...
-- Orbit update loop (only runs when orbit toggle is true)
    ...
    LeftGroupBox:AddToggle('Silent Aim', {
        Text = 'Silent Aim',
        Default = false,
        Tooltip = 'Directs Shots To Enemys Body',
        Callback = function(Value)
            GetMouseLocation = Value
            print('[cb] UniversalSilentAim changed to:', Value)
        end
    })
...
Promo = {
    ...
    "🎭Get Tappeed🎭",
    "🎭Tappeed Best Lock🎭",
    "🎭Tappeed Best Da Hood Script🎭",
    "🎭TappeedUI On Git🎭",
    "🎭Tappeed Keyless🎭",
    "🎭Tappeed Best AutoStomp🎭",
    "🎭Tappeed Best Animations🎭"
}
...
-- Define the notification function
    ...
    function cout(watermark, message)
        ...
        print('['..watermark..'] DH skin changer made by Youss')
        
        -- Show the notification
        ShowNotification("Skin Changer Loading", "Tappeed Changer")
    end
    
    -- Example usage
    cout("Tappeed Changer")           
    ...
    local function onKeyPress(input, gameProcessed)
        if not gameProcessed then
            if _G.autoStomp and input.KeyCode == stompKey then
                ...
                if isLooping then
                    showNotification("Auto Stomp Enabled", "🎭  Tappeed 🎭", 3)
                    startStomping()  -- Now calling startStomping properly
                else
                    showNotification("Auto Stomp Disabled", "🎭  Tappeed 🎭", 3)
                    stopStomping()
                end
            end
        end
    ...
    local function teleportAndStomp(targetPlayer)
        if targetPlayer and targetPlayer.Character then
            ...
            if targetHumanoid and torso and playerRoot then
                ...
                showNotification("Stomping", "🎭 Tappeed 🎭", 3)
            ...
            showNotification("No Target Found", "🎭  Tappeed 🎭", 3)
        end
    end
        ...
        elseif Config.enabled and (Config.spyOnMyself == true or p ~= player) then
            ...
            if hidden and Config.enabled then
                if Config.public then
                    saymsg:FireServer((Config.publicItalics and "/me " or '') .. "{Tappeed} [" .. p.Name .. "]: " .. msg, "All")
                else
...
LeftGroupBox = Tabs.Misc:AddLeftGroupbox('Avatar Forcefield')
    ...
    local currentColor = Color3.fromRGB(255, 255, 255) -- Default forcefield color
...
WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('Tappeed V2 | %s fps | %s ms'):format(
        math.floor(FPS),
        ping
    ))
end)
